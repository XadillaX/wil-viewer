...
function GridCell({ idx }: { idx: number }) {
  const [ src, setSrc ] = useState(defaultImg);

  useEffect(() => {
    const { cache } = common;
    if (cache.has(idx)) {
      setSrc(cache.get(idx));
    } else {
      ipcRenderer.invoke('dump-bmp', idx, true).then(ret => {
  ...
  return (
    <Image
      src={src}
      preview={false}
      style={{ maxWidth: '50px', maxHeight: '50px', position: 'relative', top: '-1px' }}
    />
  );
}
